...
//
// PR-1f reshape: the catalog is now keyed by `bundle_id: u32` (the
// value the embedded BundleComponent returns from register), not by a
// local u8 enum. The new fields drop `name`, `enabled`, and `id` in
// favor of bundle's own price/payment_token state. `price_paper`
// survives as a stopgap rewarder burn input until PR-1f-followup wires
// the actual swap+burn flow.
...
fn test_field_round_trip() {
    ...
    let pack = StarterpackImpl::new(42_u32, 4, 11, 12, 13, 14, 4, 7_680_000);
    ...
    assert!(pack.price_paper == 7_680_000, "price_paper estimate");
}
...
fn test_naked_pack_uses_zero_gear() {
    ...
    let pack = StarterpackImpl::new(1_u32, 1, 0, 0, 0, 0, 1, 990_000_000_000_000_000_000);
