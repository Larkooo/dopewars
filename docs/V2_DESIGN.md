...
| PR-1d | **in progress** | Purchase contract (`systems::purchase`). PAPER as entry currency: buyer transfers PAPER, contract burns it, mints Hustler, writes HustlerInstance. `dojo_init` seeds the 4 packs (Naked / Street / Dealer / Kingpin) using the discount formula `stake × base × (100 - stake) / 100`. Admin entrypoints for `register_starterpack` and `set_pack_enabled`. Adds `price_paper: u128` to the Starterpack model. |
| PR-1e | not started | Refactor Game model + season_manager + register_score for mint-against-supply. First call site for `Rewarder::amount` against the live PAPER supply. Strip `Season` to leaderboard fields. Delete `_stubs` crate. Wire game contract reads of `HustlerInstance` to seed runs. |
...
| PR-2  | not started | Cairo tests: starterpack registration, purchase execution, hustler minting, register_score reward minting. |
| PR-3  | not started | Web: `useConfig`, `useStarterpacks`, `useMultiplier`, reward curve chart, pack picker UI. |
| PR-4  | not started | Content: 4 hustler templates + 12 gear templates (data + art). Pack catalog seeding script. |
