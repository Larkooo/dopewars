...
const rewardFor = (supply: bigint, scoreNum: bigint) =>
  Rewarder.amount(scoreNum, 1n, MAX_SCORE, multiplierFor(supply, BURN_WEI, AVERAGE_SCORE));
...
describe("Rewarder", () => {
  ...
  describe("supplyMultiplier", () => {
    it("returns precision (1×) when supply == target", () => {
      expect(Rewarder.supplyMultiplier(TARGET_SUPPLY, TARGET_SUPPLY)).toBe(MULTIPLIER_PRECISION);
    });

    it("returns 2× precision when supply == 0", () => {
      expect(Rewarder.supplyMultiplier(0n, TARGET_SUPPLY)).toBe(2n * MULTIPLIER_PRECISION);
    });

    it("returns 0 when supply > 2× target", () => {
      expect(Rewarder.supplyMultiplier(TARGET_SUPPLY * 2n + 1n, TARGET_SUPPLY)).toBe(0n);
    });
    ...
    it("decreases linearly with supply", () => {
      ...
      const oneAndHalf = Rewarder.supplyMultiplier((TARGET_SUPPLY * 3n) / 2n, TARGET_SUPPLY);
  ...
  describe("burnMultiplier", () => {
    ...
    it("scales linearly with burn amount", () => {
      const single = Rewarder.burnMultiplier(BURN_WEI, AVERAGE_SCORE, 1n, MAX_SCORE);
      const double = Rewarder.burnMultiplier(BURN_WEI * 2n, AVERAGE_SCORE, 1n, MAX_SCORE);
